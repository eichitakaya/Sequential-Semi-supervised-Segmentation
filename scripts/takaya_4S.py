...
class SequentialSemiSupervisedSegmentation:
    def __init__(self, model, repeat_num, raw_model="", lr=0.001, _lambda=0.0005, M=3, epoch=100, batch=3, gpu_id=-1, dataset_name=0, scratch=0, pp=1, save_dir="", supervise=0, reverse=0, locally=0):
        # M == batch
        self.gpu_id = gpu_id
    ...
    def output_t(self, raw, target, predict, patient_id, slice_num):
        ...
        os.makedirs(f"../result/patient_{patient_id}", exist_ok=True)
        output.save(self.save_dir + "../result/" + f"patient_{patient_id}" + "/" + "{0:03d}".format(slice_num) + ".png")
