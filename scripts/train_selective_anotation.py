# parseで渡せるようにしたい．
from takaya_4S import SequentialSemiSupervisedSegmentation
...
for i in range(args.repeat):
    ...
    model = networks.UNet()
    learn = SemisuperviesedLearning(model, i+1, raw_model=raw_model, gpu_id=args.gpu_id, lr=args.lr, _lambda=args._lambda, n=args.n, M=args.M, epoch=args.epoch, semi_epoch=args.semi_epoch, batch=args.batch, dataset_num=args.dataset_num, scratch=args.scratch, pp=args.pp, save_dir=args.model_path, supervise=args.only_supervise, reverse=args.reverse, locally=args.locally)
    learn.training()
