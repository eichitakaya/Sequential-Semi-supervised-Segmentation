...
iou_list = []
for i in range():# 症例数だけ繰り返し
    os.makedirs(args.model_path+"/result_img"+str(i+1), exist_ok=True)
    print("repeat " + str(i+1))
    model = networks.UNet()
    learn.training(4)
    learn = SemisuperviesedLearning(model, 1, raw_model=raw_model, gpu_id=args.gpu_id, lr=args.lr, _lambda=args._lambda, n=args.n, M=args.M, epoch=args.epoch, semi_epoch=args.semi_epoch, batch=args.batch, dataset_num=args.dataset_num, scratch=args.scratch, pp=args.pp, save_dir=args.model_path, supervise=args.only_supervise, reverse=args.reverse, locally=args.locally)
    learn.training()
